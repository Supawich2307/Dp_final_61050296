...
import model.*;
import model.ItfDataControl.State;
...
public class functions implements functionOfApp,controller.ObjController{
    ...
    @Override
    public String searchTransection(Transection_flow.State state) {
        
        String Transection_ID;
        System.out.print("กรุณากรอกหมายเลขธุรกรรม => "); Transection_ID = sc.next();
        ArrayList<Transection_flow<String,model.ItfDataControl.State>> ans = searchC.search(Transection_ID,state);
    ...
    @Override
    public String searchForwardTransection() {
        String Transection_ID;
        System.out.print("กรุณากรอกหมายเลขธุรกรรม => "); Transection_ID = sc.next();
        ArrayList<Transection_flow<String,model.ItfDataControl.State>> search_Rs = searchC.searchForward(Transection_ID);
        String ans = "";
        if(search_Rs.isEmpty()){
            return ans = "ไม่มีหมายเลขธุรกรรมนี้\nกรุณาลองใหม่อีกครั้ง\n";
        }else{
            for(Transection_flow<String, State> buildString : search_Rs){
                ans = buildString.transection_id+" "+String.valueOf(buildString.state); 
            }
            return ans;
        }
