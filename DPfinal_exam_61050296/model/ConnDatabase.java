...
public class ConnDatabase implements ItfDataControl{
    ...
    public ArrayList<Transection_flow<String,State>> searchTransection(String transection_ID,model.ItfDataControl.State state) {
        ...
        try{
            ...
            while(myRs.next()){
                ...
                model.ItfDataControl.State stateRs = Enum.valueOf(model.ItfDataControl.State.class,myRs.getString(2));
                Qflow.add(new Transection_flow(FTransection_ID,stateRs));
            }
            myStmt.close();
            myRs.close();
            return Qflow;
    ...
    public ArrayList<Transection_flow<String,State>> searchForward(String transection_ID) {
        String sql = "SELECT * FROM dp_transection.transection_flow as a\n"+
                     "WHERE a.Transection_flowID < (SELECT b.Transection_flowID\n"+
                                                   "FROM dp_transection.transection_flow as b\n"+
                                                   "WHERE FTransection_ID = '"+transection_ID+"');";
        try{
            conn = DriverManager.getConnection(url,name,pass);
            Statement myStmt = conn.createStatement();
            ResultSet myRs = myStmt.executeQuery(sql);
